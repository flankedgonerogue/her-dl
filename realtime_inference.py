...
def parse_args():
    ...
    parser.add_argument(
        "--camera", type=int, default=0, help="Camera device index (0=default webcam)"
    )
    parser.add_argument(
        "--scale_factor",
        type=float,
    ...
    parser.add_argument(
        "--min_neighbors", type=int, default=5, help="Haar Cascade minNeighbors (3-8)"
    )
    parser.add_argument(
        "--min_face_size", type=int, default=60, help="Minimum face size in pixels"
    )
    parser.add_argument(
        "--demo",
        action="store_true",
...
def preprocess_face(face_roi_gray):
    ...
    face_resized = cv2.resize(
        face_roi_gray, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_AREA
    )
...
def draw_face_annotation(frame, x, y, w, h, emotion, confidence, all_probs):
    ...
    cv2.rectangle(
        frame, (x, label_y - th - 5), (x + tw + 8, label_y + baseline), color, -1
    )
    ...
    if bar_x + 120 < frame.shape[1]:
        for rank, cls_idx in enumerate(top3_idx):
            ...
            # Background
            cv2.rectangle(
                frame, (bar_x, by), (bar_x + 80, by + bar_height), (50, 50, 50), -1
            )
            # Filled bar
            if bar_width > 0:
                cv2.rectangle(
...
def draw_hud(frame, fps, n_faces, model_trained, paused=False):
    ...
    cv2.putText(
        frame, f"Model: {model_str}", (8, 62), font, 0.45, status_color, 1, cv2.LINE_AA
    )
    cv2.putText(
        frame,
...
def generate_demo_frame(width=640, height=480):
    ...
    cv2.ellipse(
        frame, (cx, cy), (face_w // 2, face_h // 2), 0, 0, 360, (180, 160, 140), -1
    )
    # Eyes
